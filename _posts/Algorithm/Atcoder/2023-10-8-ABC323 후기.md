...
문제 한번 더럽게 길다. 솔직히 30분 중에 이해하는데 10분 넘게 쓴거 같다. 게다가 처음 넣은 답은 틀린 답이었다. 처음 제출한 답은 풀이 접근 자체는 맞는 접근이었다. 그러나 다른 풀었던 문제를 다시 푸는 경우도 생각해버려서 이미 푼 문제를 따로 기록해야 함을 깨달았다. 이후 풀지 않은 문제들(풀 수 있는 문제들)을 ```none_solve``` 리스트에 저장해서 해결했다.
